...
describe('validateImplementation — getGameMetadata fields', () => {
  ...
  test('rejects missing or non-positive estimatedDurationMinutes', () => {
    expect(() =>
      validateImplementation(withMeta({ ...ok, estimatedDurationMinutes: undefined })),
    ).toThrow(/estimatedDurationMinutes/);
    expect(() => validateImplementation(withMeta({ ...ok, estimatedDurationMinutes: 0 }))).toThrow(
      /estimatedDurationMinutes/,
    );
    expect(() => validateImplementation(withMeta({ ...ok, estimatedDurationMinutes: -5 }))).toThrow(
      /estimatedDurationMinutes/,
    );
    expect(() =>
      validateImplementation(withMeta({ ...ok, estimatedDurationMinutes: NaN })),
    ).toThrow(/estimatedDurationMinutes/);
    expect(() =>
      validateImplementation(withMeta({ ...ok, estimatedDurationMinutes: '10' })),
    ).toThrow(/estimatedDurationMinutes/);
  });

  test('rejects invalid complexity values', () => {
    expect(() => validateImplementation(withMeta({ ...ok, complexity: undefined }))).toThrow(
      /complexity/,
    );
    expect(() => validateImplementation(withMeta({ ...ok, complexity: 'extreme' }))).toThrow(
      /complexity/,
    );
    expect(() => validateImplementation(withMeta({ ...ok, complexity: 'LIGHT' }))).toThrow(
      /complexity/,
    );
  });
  ...
  test('rejects tags containing non-strings or empty strings', () => {
    expect(() => validateImplementation(withMeta({ ...ok, tags: ['ok', '', 'also'] }))).toThrow(
      /tags/,
    );
    expect(() => validateImplementation(withMeta({ ...ok, tags: ['ok', 42] }))).toThrow(/tags/);
    expect(() => validateImplementation(withMeta({ ...ok, tags: ['ok', null] }))).toThrow(/tags/);
  });
